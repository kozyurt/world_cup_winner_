"""
spain_argentina.py — World Cup 2026 Final predictor: Spain vs Argentina
...
MATCH_NEUTRAL = True      # 2026 final at a neutral US venue for these teams

ELO_BASE = 1500.0
ELO_K = 32
ELO_HOME_BONUS = 60

# palette (matches predict_today.py's reel charts)
INK, MUTE, GRID = "#1a1a2e", "#8a8a9e", "#e8e8ee"
ORANGE, BLUE, GRAY = "#ff6b18", "#1f6feb", "#9aa0a6"

# national-team colors for the win-probability bars, keyed by team name
# so the right color follows the team even if HOME/AWAY order changes.
TEAM_COLORS = {
    "Spain": "#C60B1E",       # La Roja (flag red)
    "Argentina": "#75AADB",   # La Albiceleste (celeste / sky blue)
}
DRAW_GRAY = "#9aa0a6"          # neutral, belongs to neither side
DEFAULT_TEAM_COLOR = "#1f6feb"  # fallback for any team not in TEAM_COLORS
plt.rcParams.update({
...
# ── fixture lookup (just the Final) ─────────────────────────────────────────────
def find_final_fixture():
    """Look up the Spain v Argentina row in data_cache/fixtures.csv."""
    fx = pd.read_csv(FIXTURES_PATH)
    for _, row in fx.iterrows():
        if " v " not in str(row["teams"]):
            continue
        left, right = [p.strip() for p in str(row["teams"]).split(" v ")]
        if {left, right} == {HOME_TEAM, AWAY_TEAM}:
    ...
    raise SystemExit(
        f"Couldn't find a '{HOME_TEAM} v {AWAY_TEAM}' row in {FIXTURES_PATH}. "
        "Update the Final's row in fixtures.csv first."
    )
...
def make_chart(m, p_home, p_draw, p_away, slate_date, out_dir):
    ...
    colors = [
        TEAM_COLORS.get(m["home_disp"], DEFAULT_TEAM_COLOR),
        DRAW_GRAY,
        TEAM_COLORS.get(m["away_disp"], DEFAULT_TEAM_COLOR),
    ]
...
def tag_match(top_prob, p_home, p_away, home_elo, away_elo):
    ...
    if top_prob >= 0.60:
        ...
        strength = "TOSS-UP"
    return strength + ("  UPSET PICK" if upset else "")


# ── main ────────────────────────────────────────────────────────────────────────
def main():
    print(f"\nLoading data + building features for {HOME_TEAM} vs {AWAY_TEAM} ...")
    results = load_results()
    dataset, final_elo = build_dataset(results)
    long = per_team_long(results)

    m = find_final_fixture()
    match_date = m["date"]
    ...
    p_home, p_draw, p_away = predict_symmetric(
        model, long, final_elo, HOME_TEAM, AWAY_TEAM, match_date, MATCH_NEUTRAL, MATCH_WEIGHT)
    outcomes = [(m["home_disp"], p_home), ("Draw", p_draw), (m["away_disp"], p_away)]
    pick, conf = max(outcomes, key=lambda x: x[1])
    he, ae = final_elo.get(HOME_TEAM, ELO_BASE), final_elo.get(AWAY_TEAM, ELO_BASE)
    tag = tag_match(conf, p_home, p_away, he, ae)

    out_dir = os.path.join("prediction", str(match_date))
